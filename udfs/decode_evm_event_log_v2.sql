...
def decode_evm_event_log_v2(topic_data: str, log_data: str, parse_json) -> dict:
    ...
    chunk_size = 64
    byte_array = [
        log_data[i : i + chunk_size] for i in range(0, len(log_data), chunk_size)
    ]
    topic_byte_array = [
        topic_data[i : i + chunk_size] for i in range(0, len(topic_data), chunk_size)
    ]
    complete_byte_array = []
    input_types = []
    input_names = []
    ...
    for input in parse_json["inputs"]:
        if input["type"] in acceptable_types or "uint" in input["type"] or "bytes" in input["type"]:
            ...
            if not input["indexed"] and len(byte_array) > 0:
    ...
    for idx, byte_entry in enumerate(complete_byte_array):
        try:
            ...
            elif "bytes" in input_types[idx]:
                decoded.append(byte_entry)
        except Exception:
            continue
