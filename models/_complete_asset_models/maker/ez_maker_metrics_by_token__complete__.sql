...
left join accounting_liq l
    ...
    and a.token = l.token
),  __dbt__cte__fact_maker_fees_revenue_expenses_by_token as (
...
GROUP BY 1
        ...
        SELECT
            ...
            operating_expenses,
            total_expenses
        FROM __dbt__cte__fact_maker_fees_revenue_expenses_by_token
    )
    , treasury AS (
            SELECT
                date,
    ...
    , treasury_native AS (
        SELECT date, token, amount_mkr as treasury_native FROM __dbt__cte__fact_treasury_mkr
    )
    , net_treasury AS (
            SELECT
                date,
    ...
    , tvl_metrics AS (
        ...
        FROM __dbt__cte__fact_maker_tvl_by_asset
    )
    , outstanding_supply AS (
        SELECT date, 'DAI' as token, outstanding_supply FROM __dbt__cte__fact_dai_supply
    )
