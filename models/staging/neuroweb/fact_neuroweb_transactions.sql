with
    transactions as (
        {{ parse_parity_parquets("neuroweb", "transactions") }}
    ),
    fees as (
        {{ parse_parity_parquets("neuroweb", "fees") }}
    ),
    decimals as (
        select
            chain
        ...
        where chain = 'neuroweb'
    ),
    prices as ({{ get_coingecko_price_with_latest("neurowebai") }})
    select
