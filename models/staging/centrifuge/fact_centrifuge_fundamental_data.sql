with
    transactions as (
        {{ parse_parity_parquets("centrifuge", "transactions") }}
    ),
    fees as (
        {{ parse_parity_parquets("centrifuge", "fees") }}
    ),
    decimals as (
        select
            chain
        ...
        where chain = 'centrifuge'
    ),
    prices as ({{ get_coingecko_price_with_latest("centrifuge") }})
    select
