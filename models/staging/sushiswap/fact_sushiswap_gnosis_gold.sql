...
with
    all_metrics as (
        ...
        from {{ ref("fact_sushiswap_v2_tvl_gnosis") }} t1
        full outer join
            {{ ref("fact_sushiswap_v2_unique_traders_gnosis") }} t2 on t1.date = t2.date
        full outer join
            {{ ref("fact_sushiswap_v2_trading_vol_and_fees_gnosis") }} t3
            on t1.date = t3.date
    )
