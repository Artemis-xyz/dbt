...
with
    chain_tvl as (
        ...
        from {{ ref("fact_pancakeswap_v2_tvl_ethereum") }}
        union all
        select date, chain, 'pancakeswap' as app, category, tvl
        from {{ ref("fact_pancakeswap_v3_tvl_ethereum") }}
    ),
    combined_tvl as (
    ...
    chain_unique_traders as (
        ...
        from {{ ref("fact_pancakeswap_v2_unique_traders_ethereum") }}
        union all
        select date, chain, 'pancakeswap' as app, category, unique_traders
        from {{ ref("fact_pancakeswap_v3_unique_traders_ethereum") }}
    ),
    ...
    chain_vol_fees as (
        ...
        from {{ ref("fact_pancakeswap_v2_trading_vol_and_fees_ethereum") }}
        union all
        select date, chain, 'pancakeswap' as app, category, trading_volume, fees
        from {{ ref("fact_pancakeswap_v3_trading_vol_and_fees_ethereum") }}
    ),
