...
with
    chain_tvl as (
        ...
        from {{ ref("fact_pancakeswap_v2_tvl_arbitrum") }}
        union all
        select date, chain, 'pancakeswap' as app, category, tvl
        from {{ ref("fact_pancakeswap_v3_tvl_arbitrum") }}
    ),
    combined_tvl as (
    ...
    chain_unique_traders as (
        ...
        from {{ ref("fact_pancakeswap_v2_unique_traders_arbitrum") }}
        union all
        select date, chain, 'pancakeswap' as app, category, unique_traders
        from {{ ref("fact_pancakeswap_v3_unique_traders_arbitrum") }}
    ),
    ...
    chain_vol_fees as (
        ...
        from {{ ref("fact_pancakeswap_v2_trading_vol_and_fees_arbitrum") }}
        union all
        select date, chain, 'uniswap' as app, category, trading_volume, fees
        from {{ ref("fact_pancakeswap_v3_trading_vol_and_fees_arbitrum") }}
    ),
