-- Celo data from goldsky has two data quality issues:
-- 1. version 1.3.0 has the fee_currency field but version 1.1.0 does not
-- 2. version 1.3.0 has DQ issues with the gas_price field but 1.1.0 does not
-- These issues persist until block 31056500
-- This model is used index the pre 31056500 block data.
{{
    config(
...
data as (
        ...
        from {{ source("PROD_LANDING", "raw_celo_l1_transactions_31056500_parquet") }}
        {% if is_incremental() %}
            where parquet_raw:block_timestamp::timestamp_ntz
