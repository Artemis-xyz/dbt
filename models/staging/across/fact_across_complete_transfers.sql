...
transfer_with_price as  (
    ...
    LEFT JOIN prices as src_prices
        ON lower(origin_token) = lower(src_prices.contract_address) and date_trunc('day', src_block_timestamp) = src_prices.date and src_chain = src_prices.chain
    LEFT JOIN prices as dst_prices
        ON lower(destination_token) = lower(dst_prices.contract_address) and date_trunc('day', dst_block_timestamp) = dst_prices.date and dst_chain = dst_prices.chain
)
SELECT 
