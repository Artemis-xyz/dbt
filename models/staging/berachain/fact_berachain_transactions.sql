{{ config(materialized="incremental", snowflake_warehouse="BERACHAIN", unique_key=["transaction_hash", "block_timestamp"]) }}
with
    raw_receipt_transactions as (
        ...
        from {{ source("PROD_LANDING", "raw_berachain_transactions_parquet") }}
        {% if is_incremental() %}
