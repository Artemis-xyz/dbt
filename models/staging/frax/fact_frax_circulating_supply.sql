...
frax_data AS (
  -- Parse the FRax data from the JSON response
  SELECT 
    EXTRACTION_DATE,
    r.value AS item
  FROM {{source("PROD_LANDING", "raw_frax_circulating_supply")}} a,
  LATERAL FLATTEN(input => PARSE_JSON(a.SOURCE_JSON):frax:full_items) r
...
SELECT 
  ...
  item:frax_burned_cumulative_sum::float AS frax_burned_cumulative_sum
FROM frax_data
ORDER BY date