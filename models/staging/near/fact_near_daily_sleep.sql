...
with
        ...
        select
            ...
            datediff(
                ...
                lag(block_timestamp, 1) over (
                    partition by tx_signer, raw_date order by block_timestamp
                ),
                block_timestamp
            ) as hours_last_seen
    ...
    max_hours_between_transactions as (
        select tx_signer, raw_date, max(hours_last_seen) as max_hours_last_seen
        from hours_between_transactions
        group by tx_signer, raw_date
    ),
    hours_till_since_midnight as (
        select
            ...
            datediff('hour', raw_date, first_day_transactions) as hour_since_midnight
        from near_flipside.core.fact_transactions
        {% if is_incremental() %}
            where block_timestamp >= dateadd(day, -3, to_date(sysdate()))
        {% endif %}
...
select
    max_hours_between_transactions.tx_signer as from_address,
...
left join
    ...
    on max_hours_between_transactions.tx_signer = hours_till_since_midnight.tx_signer
