...
with
    all_metrics as (
        ...
        from {{ ref("fact_uniswap_v3_tvl_polygon") }} t1
        full outer join
            {{ ref("fact_uniswap_v3_unique_traders_polygon") }} t2 on t1.date = t2.date
        full outer join
            {{ ref("fact_uniswap_v3_trading_vol_and_fees_polygon") }} t3
            on t1.date = t3.date
    )
