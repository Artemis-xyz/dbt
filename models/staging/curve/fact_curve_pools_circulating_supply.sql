...
-- Used for Convex for calculating Circulating Supply since we need price of all Curve related tokens (including LP tokens)
...
SELECT
    ...
    contract_address,
    p.symbol,
    SUM(daily_balance/POW(10, COALESCE(p.decimals,18))) as circulating_supply
FROM
    filled_balances
