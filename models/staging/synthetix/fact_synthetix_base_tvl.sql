...
daily_tvl as (
    select 
        ...
        eph.symbol as token, 
        coalesce(balance_token/pow(10,coalesce(decimals,0)), 0) * eph.price as balance_usd
...
daily_with_series as (
    select ds.date, dt.token, dt.balance_usd
    from date_series ds
...
select 
    ...
    last_value(balance_usd ignore nulls) over (
        order by date rows between unbounded preceding and current row
    ) as tvl_usd
