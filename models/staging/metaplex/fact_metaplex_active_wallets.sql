...
seller_activity AS (
    SELECT 
        ...
        s.seller AS wallet
    FROM 
        solana_flipside.nft.fact_nft_sales AS s
    JOIN 
...
recipient_activity AS (
    SELECT 
        ...
        t.tx_to AS wallet
    FROM 
        solana_flipside.core.fact_transfers AS t
    WHERE 
...
event_activity AS (
    ...
    FROM 
        solana_flipside.core.fact_events AS fe,
...
transaction_activity AS (
    SELECT 
        ...
        signer.value AS wallet
    FROM 
        solana_flipside.core.fact_transactions AS ft
    JOIN 
        solana_flipside.core.fact_events AS fe ON ft.tx_id = fe.tx_id,
