{{
    config(
        ...
        database="momentum",
        schema="core",
        alias="ez_momentum_metrics_by_token",
    )
}}
...
WITH
    ...
    , spot_fees_revenue AS (
        SELECT date, symbol_a AS token, SUM(fees_native) AS fees_native, SUM(fees_usd) AS fees_usd, SUM(service_cash_flow) AS service_cash_flow, SUM(service_cash_flow_native) AS service_cash_flow_native, SUM(foundation_cash_flow) AS foundation_cash_flow, SUM(foundation_cash_flow_native) AS foundation_cash_flow_native
        FROM {{ ref("fact_momentum_spot_fees_revenue") }}
        GROUP BY 1, 2

        UNION ALL

        SELECT date, symbol_b AS token, SUM(fees_native) AS fees_native, SUM(fees_usd) AS fees_usd, SUM(service_cash_flow) AS service_cash_flow, SUM(service_cash_flow_native) AS service_cash_flow_native, SUM(foundation_cash_flow) AS foundation_cash_flow, SUM(foundation_cash_flow_native) AS foundation_cash_flow_native
        FROM {{ ref("fact_momentum_spot_fees_revenue") }}
        GROUP BY 1, 2
    )
