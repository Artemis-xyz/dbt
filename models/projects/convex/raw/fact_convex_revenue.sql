...
, date_address_spine AS (
    SELECT
        ...
        t.contract_address
    FROM
        pc_dbt_db.prod.dim_date_spine ds
    JOIN transfers t
    WHERE ds.date between (select min(date) from transfers) and to_date(sysdate())
)
select
    das.date,
    p.symbol as token,
...
from
    ...
    left join transfers as t on t.date = das.date and t.contract_address = das.contract_address
    left join ethereum_flipside.price.ez_prices_hourly as p on p.token_address = t.contract_address
    and p.hour = das.date
group by
    1,
    2
