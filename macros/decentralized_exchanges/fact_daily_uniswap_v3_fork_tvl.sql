{% macro fact_daily_uniswap_v3_fork_tvl(token_address, chain, app) %}
    with recursive
        pools as (
            select
        ...
        viable_pools as (
            select date, pool, token0_amount_usd + token1_amount_usd as pool_tvl
            from with_price
            where
                abs(
    ...
    select date, '{{ chain }}' as chain, '{{ app }}' as app, 'DeFi' as category, tvl
    from tvl_daily_sum
    where date is not null
{% endmacro %}
